...
// script.js (добавим в конец функции updateCartCounter)
function updateCartCounter() {
    ...
    document.getElementById('cart-link').href = `cart.html?${currentParams.toString()}`;
    
    // Обновляем ссылку на заказы если она есть
    const ordersLink = document.getElementById('orders-link');
    if (ordersLink) {
        ordersLink.href = `orders.html?${currentParams.toString()}`;
    }
}
